...
func TestGenerateCommand(t *testing.T) {
	...
	// Set up args for generate command
	args := []string{"mushu", "generate"}
	...
	// Check output contains expected text
	if !strings.Contains(output, "Policy generation not yet implemented") {
		t.Errorf("Expected output to contain 'Policy generation not yet implemented', got: %s", output)
	}
}

func TestGenerateCommand_WithFlags(t *testing.T) {
	// Capture stdout
	oldStdout := os.Stdout
	r, w, _ := os.Pipe()
	os.Stdout = w
	...
	// Set up args for generate command with flags
	args := []string{"mushu", "generate", "--rules", "custom.yaml", "--output", "output.star"}
	...
	// Check output mentions the custom rules file
	if !strings.Contains(output, "custom.yaml") {
		t.Errorf("Expected output to contain 'custom.yaml', got: %s", output)
	}

	// Check output mentions the output file
	if !strings.Contains(output, "output.star") {
		t.Errorf("Expected output to contain 'output.star', got: %s", output)
	}
}
